...
import { IKernelPool } from "./interfaces/IKernelPool.sol";

contract KernelPool is IKernelPool {
    ...
    IKernelManager public override manager;
    ...
    modifier onlyKernelManager() {
        require(msg.sender == address(manager), "Only manager can call this function");
        _;
    }
    ...
    constructor(address _manager) {
        manager = IKernelManager(_manager);
    }
        ...
        lock
    {
        ...
        if (observationCardinalityNextOld != observationCardinalityNextNew) {
            manager.eventEmitter().increaseObservationCardinalityNext(
    ...
    function initialize(
        ...
        external
    {
        // TODO: make it only callable by trusted entity
        ...
        (token0, token1, fee, tickSpacing) = (_token0, _token1, _fee, _tickSpacing);
        maxLiquidityPerTick = Tick.tickSpacingToMaxLiquidityPerTick(_tickSpacing);

        manager.eventEmitter().initialize(sqrtPriceX96, tick);
    }
        ...
        returns (uint256 amount0, uint256 amount1)
    {
        ...
        manager.eventEmitter().mint(
            recipient, recipient, tickLower, tickUpper, amount, amount0, amount1
        );
        ...
        returns (uint128 amount0, uint128 amount1)
    {
        ...
        manager.eventEmitter().collect(recipient, recipient, tickLower, tickUpper, amount0, amount1);
    }
        ...
        returns (uint256 amount0, uint256 amount1)
    {
        ...
        manager.eventEmitter().burn(from, tickLower, tickUpper, amount, amount0, amount1);
        ...
        returns (int256 amount0, int256 amount1)
    {
        ...
        manager.eventEmitter().swap(
            recipient, recipient, amount0, amount1, state.sqrtPriceX96, state.liquidity, state.tick
        );
        slot0.unlocked = true;

        if (readOnly) {
            uint160 sqrtPriceAfter = slot0.sqrtPriceX96;
            assembly {
                let ptr := mload(0x40)
                mstore(ptr, amount0)
                mstore(add(ptr, 0x20), amount1)
                mstore(add(ptr, 0x40), sqrtPriceAfter)
                revert(ptr, 0x60)
            }
        }
    }

    // TODO: add protocol fee to the pool, currently there is incorerect access restriction
        ...
        onlyKernelManager
    {
        ...
        manager.eventEmitter().setFeeProtocol(
        ...
        returns (uint128 amount0, uint128 amount1)
    {
        ...
        manager.eventEmitter().collectProtocol(recipient, recipient, amount0, amount1);
    }
}
